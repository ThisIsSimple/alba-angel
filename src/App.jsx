...
function App() {
  ...
  const [products, setProducts] = useState([]); // 전체 상품
  const [filteredProducts, setFilteredProducts] = useState([]); // 실제 선택된 상품
  const [searchedProducts, setSearchedProducts] = useState([]); // 보여지는 상품
  ...
  const requestProducts = () => {
    setLoading(true);
    setButtonLoading(true);
    setLoadingText(`${filteredBrands.length}개의 필터로 상품을 찾는 중...`);
    axios
  ...
  const requestProductsDetail = async () => {
    ...
    setLoadingText(`${filteredProducts.length}개 상품의 판매량 조회중`);
    const newProducts = [];
    for (const product of filteredProducts) {
      ...
      if (!isDetailChecked) {
        try {
          const res = await axios.post(`${BASE_URL}/api/productDetail`, {
            url,
          });
  ...
  const searchProducts = (e, reset = false) => {
    ...
    if (reset) setSearchedProducts(products);
    else
      setSearchedProducts(products.filter(({ name }) => name.includes(query)));
  };
  ...
  return (
    <>
        ...
        <section className="mb-4">
          <header className="flex justify-between items-start mb-3">
            <div className="flex items-center">
              <h2 className="font-bold mr-3">상품 목록 ({products.length})</h2>
              {products.length > 0 && (
                <>
                  <button
                    className="bg-blue-500 text-white rounded text-sm px-2 py-0.5 mr-2"
                    onClick={() =>
                      setFilteredProducts(
                        _.union(filteredProducts, searchedProducts)
                      )
                    }
                  >
                    전체 선택
                  ...
                  <button
                    ...
                    onClick={() =>
                      setFilteredProducts(
                        _.reject(filteredProducts, (item) =>
                          _.find(searchedProducts, { id: item.id })
                        )
                      )
                    }
                  >
            ...
            <div className="mb-1">
              <form onSubmit={searchProducts} className="flex items-center">
          ...
          {searchedProducts.length > 0 ? (
            searchedProducts.map((product) => {
              ...
              const active = filteredProducts.some((fp) => fp.id === id);
              return (
                <div
                  key={name}
                  className="flex flex-nowrap items-center bg-gray-200 rounded-lg p-3 mb-3"
                >
                  <div
                    className={`${
                      active ? "bg-blue-500" : "bg-white"
                    } flex-shrink-0 w-6 h-6 border-4 rounded border-white cursor-pointer mr-2`}
                    onClick={() => {
                      if (active) {
                        setFilteredProducts(
                          filteredProducts.filter((fp) => fp.id !== id)
                        );
                      } else {
                        setFilteredProducts(filteredProducts.concat(product));
                      }
                    }}
                  />
                  <div
      ...
      <div className="h-16" />
      <footer className="grid grid-cols-2 fixed bottom-0 left-0 w-screen p-3 bg-white shadow border-t border-gray-400">
        <div>
          <div className="mb-1">
            <span className="mr-3">
              <span className="bg-gray-300 text-sm px-2 py-0.5 rounded mr-2">
                카테고리
              </span>
              {categoryId}
            </span>
          ...
          <button
            ...
            onClick={() => {
              ...
              setSearchedProducts(
                _.reject(
                  searchedProducts,
                  (item) => !_.find(filteredProducts, { id: item.id })
                )
              );
            }}
            disabled={buttonLoading}
